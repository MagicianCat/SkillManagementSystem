...
public class WorkflowController {
 ...
 @PostMapping("/skill-versions/{id}:push-to-company") WorkflowService.ReviewView pushToCompany(@PathVariable Long id,@RequestBody(required=false) CommentRequest r,Authentication a,HttpServletRequest h){return service.pushToCompany(id,r==null?null:r.comment,(Long)a.getPrincipal(),h.getRequestId());}
 @GetMapping("/reviews") PageResponse<WorkflowService.ReviewView> reviews(@RequestParam(required=false)ReviewStatus status,@RequestParam(required=false)String keyword,Pageable pageable){return PageResponse.from(service.reviews(status,keyword,pageable),v->v);}
 ...
 public record VersionAction(@NotNull Integer versionNo,@NotBlank String comment){}public record CommentRequest(@NotBlank String comment){}public record BatchReviewRequest(@NotEmpty@Size(max=50)java.util.List<@NotNull Long> reviewIds,@Size(max=2000)String comment){}public record DeprecateRequest(Long replacementVersionId){}public record OfflineRequest(boolean force,@NotBlank String reason){}
}
