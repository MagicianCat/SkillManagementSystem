...
public class BrowserSessionAuthenticationFilter extends OncePerRequestFilter {
    ...
    @Override protected void doFilterInternal(HttpServletRequest request,HttpServletResponse response,FilterChain chain) throws ServletException,IOException {
        if (SecurityContextHolder.getContext().getAuthentication() == null) {
            ...
            if(raw!=null) try { var u=sessions.restore(raw).user(); var permissions=new java.util.HashSet<>(u.permissions()); scopedRoles.findByUserId(u.id()).forEach(a -> { if (java.util.Set.of("TEAM_ADMIN","TEAM_MAINTAINER","PLATFORM_MAINTAINER").contains(a.getRoleKey())) permissions.addAll(java.util.List.of("skill:browse","skill:download","skill:upload","skill:edit")); if ("PROJECT_MANAGER".equals(a.getRoleKey())) permissions.add("project:create"); if ("TEAM_ADMIN".equals(a.getRoleKey())) { permissions.add("skill:review"); permissions.add("admin:telemetry"); } }); var authorities=permissions.stream().map(org.springframework.security.core.authority.SimpleGrantedAuthority::new).toList(); SecurityContextHolder.getContext().setAuthentication(new UsernamePasswordAuthenticationToken(u.id(),null,authorities)); } catch(BusinessException ignored) { }
        }
