...
public class MinioObjectStorageAdapter implements ObjectStoragePort {
    ...
    @Override public InputStream get(String key, long offset, long length) { try { return client.getObject(GetObjectArgs.builder().bucket(properties.bucket()).object(key).offset(offset).length(length).build()); } catch (Exception ex) { log.error("event=storage.get_range.failed bucket={} key={} offset={} length={} errorCode=STORAGE_READ_FAILED",properties.bucket(),key,offset,length,ex); throw new BusinessException("STORAGE_READ_FAILED", "Unable to read source revision", HttpStatus.BAD_GATEWAY); } }
    @Override public StorageObjectMetadata stat(String key) { try { var response = client.statObject(StatObjectArgs.builder().bucket(properties.bucket()).object(key).build()); return new StorageObjectMetadata(response.size(), response.contentType(), response.etag()); } catch (Exception ex) { log.error("event=storage.stat.failed bucket={} key={} errorCode=STORAGE_READ_FAILED",properties.bucket(),key,ex); throw new BusinessException("STORAGE_READ_FAILED", "Unable to inspect source revision", HttpStatus.BAD_GATEWAY); } }
