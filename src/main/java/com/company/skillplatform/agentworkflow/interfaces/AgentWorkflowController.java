...
public class AgentWorkflowController {
 ...
 @PostMapping("/projects/{projectKey}/workflow-runs") public AgentWorkflowService.RunView start(@PathVariable String projectKey,@RequestBody RunRequest r,Authentication a){return service.start(projectKey,actor(a),new AgentWorkflowService.RunCommand(r.workflowCode()==null?"requirement-mvp":r.workflowCode(),r.workflowVersion()==null?1:r.workflowVersion(),r.profileVersionIds()==null?List.of():r.profileVersionIds()));}
 ...
 @PostMapping("/workflow-runs/{runId}/interventions") public AgentWorkflowService.InterventionView intervention(@PathVariable Long runId,@RequestBody InterventionRequest r,Authentication a){return service.intervene(runId,actor(a),new AgentWorkflowService.InterventionCommand(r.type(),r.content()));}
 ...
 @GetMapping(value="/workflow-runs/{runId}/events",produces=MediaType.TEXT_EVENT_STREAM_VALUE) public SseEmitter events(@PathVariable Long runId,Authentication a){service.run(runId,actor(a));SseEmitter e=new SseEmitter(30000L);try{e.send(SseEmitter.event().name("workflow.status.changed").id(String.valueOf(runId)).data(service.run(runId,actor(a))));e.complete();}catch(Exception x){e.completeWithError(x);}return e;}
 private Long actor(Authentication a){return (Long)a.getPrincipal();}
 public record ProfileRequest(String code,String name,String category,String description){} public record VersionRequest(String systemPrompt,String modelCode,Double temperature,Integer maxIterationPerRun,Integer timeoutSeconds,String outputSchemaJson,String runtimeConfigJson,String changelog,List<Long> skillIds,List<AgentWorkflowService.ToolBinding> tools){} public record RunRequest(String workflowCode,Integer workflowVersion,List<Long> profileVersionIds){} public record InterventionRequest(String type,String content){} public record AcceptanceRequest(String decision,String comment){}
}
