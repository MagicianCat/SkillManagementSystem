...
public class AgentWorkflowController {
 ...
 @PostMapping("/agent-profiles/{code}:fork") public AgentWorkflowService.ProfileView fork(@PathVariable String code,@RequestBody ForkRequest r,Authentication a){return service.fork(code,actor(a),r.newCode(),r.newName(),r.versionNo());}
 ...
 @PostMapping("/workflow-runs/{runId}/human-questions/{questionId}:answer") public AgentWorkflowService.HumanQuestionView answer(@PathVariable Long runId,@PathVariable Long questionId,@RequestBody HumanAnswerRequest r,Authentication a){return service.answerHumanQuestion(runId,questionId,actor(a),r.answer());}
 ...
 public record ProfileRequest(String code,String name,String category,String description){} public record ForkRequest(String newCode,String newName,Integer versionNo){} public record VersionRequest(String systemPrompt,String modelCode,Double temperature,Integer maxIterationPerRun,Integer timeoutSeconds,String outputSchemaJson,String runtimeConfigJson,String changelog,List<AgentWorkflowService.SkillBinding> skills,List<AgentWorkflowService.ToolBinding> tools){} public record RunRequest(String initialRequest,com.fasterxml.jackson.databind.JsonNode contextSnapshotJson){} public record InterventionRequest(String type,String content,Long stageRunId,Long agentSessionId,Long agentRunId){} public record HumanAnswerRequest(String answer){} public record AcceptanceRequest(String decision,String comment){} public record DesignAcceptanceRequest(String decision,String targetStageKey,String comment){}
}
